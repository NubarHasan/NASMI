...
_EXTRACTOR_ID: ExtractorId = ExtractorId("de.passport.v1")
...
class PassportExtractor(GermanDocumentExtractor):
    ...
    def _extract_from_mrz(
        ...
        *,
        ...
        if not mrz_span_ids:
            _log.warning(
                "passport extractor: no spans matched MRZ anchor %r — facts will have no span",
                anchor,
            )
        ...
        if "personal_number" in requested:
        ...
        *,
        ...
        for fact_type, patterns in _OCR_LABEL_PATTERNS.items():
            ...
            for pattern in patterns:
                ...
                if not span_ids:
                    _log.debug(
                        "passport extractor: no span matched OCR value %r for %r",
                        raw,
                        fact_type,
                    )
                    continue
